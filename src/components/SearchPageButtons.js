...
class SearchPageButtons extends Component {
  ...
  handleClick = (e) =>{
    if (e.target.className === "change_page") {
      ...
      this.props.history.push(`/movies/search/${this.props.match.params.term}/${Number(e.target.textContent)}`)
    }else if(e.target.className === "decrement_page"){
      this.props.decrementPage(this.props.page)
      this.props.history.push(`/movies/search/${this.props.match.params.term}/${this.props.page-1}`)
      ...
      this.props.history.push(`/movies/search/${this.props.match.params.term}/${1}`)
    }else if(e.target.className === "last_page"){
      this.props.lastPage(this.props.maxPages)
      this.props.history.push(`/movies/search/${this.props.match.params.term}/${this.props.maxPages}`)
    }
  }
...
export default withRouter(connect(mapStateToProps, mapDispatchToProps)(SearchPageButtons));
